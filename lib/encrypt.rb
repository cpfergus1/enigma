class Encrypt
  ...
  def encrypt(message, cipherkey = nil, cipherdate = nil)
    ...
    { encryption: encryption(message, keys), key: cipherkey, date: cipherdate }
  end

  def encryption(message, key_hash)
    encoded_message = []
    iter = 0
    loop do
      break if iter > message.length

      encoded_message << key_hash[@key_a[iter % 4]][message.chars[iter]]
      iter += 1
    end
